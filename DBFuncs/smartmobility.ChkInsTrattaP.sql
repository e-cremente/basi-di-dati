...
DROP FUNCTION IF EXISTS smartmobility.ChkInsTrattaP;

delimiter //
CREATE DEFINER=`root`@`localhost` FUNCTION `ChkInsTrattaP`(
pCodStrada varchar(32),
pNumCarreggiata tinyint,
pLunghezza decimal (9,3),
pChilometroIni decimal(9,3),
pLatitudineIni decimal(10,7),
pLongitudineIni decimal(10,7),
pChilometroFin decimal(9,3),
...
ChkInsTrattaP:BEGIN
    ...
    if lvRecNotFound = 1 then
        ...
        set lvCodPosizioneIni = ChkInsPosizione(pLatitudineIni, pLongitudineIni, pCodStrada, pChilometroIni, NULL);
        set lvCodPosizioneFIn = ChkInsPosizione(pLatitudineFin, pLongitudineFin, pCodStrada, pChilometroFin, NULL);
        insert into Tratta(
