DROP TRIGGER IF EXISTS Fruibilita_BEFORE_UPDATE;

DELIMITER $$

CREATE DEFINER = CURRENT_USER TRIGGER `smartmobility`.`Fruibilita_BEFORE_UPDATE` BEFORE UPDATE ON `Fruibilita` FOR EACH ROW
BEGIN
